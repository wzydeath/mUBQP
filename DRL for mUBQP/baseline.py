...
class RolloutBaseline:
	def __init__(self, cfg, model, device = 'cpu',
		...
		self.n_rollout_samples = cfg.n_rollout_samples
		self.cur_epoch = epoch
		self.wp_epochs = cfg.wp_epochs
		self.beta = cfg.warmup_beta
		...
		self.n_customer = cfg.n_customer
		self.weight_dir = cfg.weight_dir

		self.device = device
		self.model = AttentionModel(cfg.embed_dim, cfg.n_encode_layers, cfg.n_heads, cfg.tanh_clipping)
	...
	def _update_baseline(self, model, epoch):
		...
		self.bl_vals = self.eval(self.dataset.data).cpu().numpy()
	...
	def epoch_callback(self, model, epoch):
		...
		candidate_vals, _ = model(self.dataset.data, decode_type = 'greedy')
